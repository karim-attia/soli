...
function applyMoveInPlace(state, move) {
  ...
  if (move.kind === 't2t') {
    ...
    let flipped = false
    if (newTopIndex >= 0 && !state.tableau[move.from][newTopIndex].faceUp) {
      state.tableau[move.from][newTopIndex].faceUp = true
      flipped = true
    }
...
function tryGreedyFinishIfReady(state) {
  ...
  const greedy = {
    ...
    foundations: (() => {
      const f = {}
      for (const s of SUITS) f[s] = state.foundations[s].map((x) => ({ ...x }))
      return f
    })(),
...
function solveKlondike(deckOrder, options = {}) {
  ...
  if (tryGreedyFinishIfReady(state)) {
    return {
      solvable: true,
      difficulty: classifyDifficulty(true, 0),
      stats: { nodes: 0, depth: 0, timeMs: 0, cutoffReason: null },
    }
  }
  if (!pushFrame(0)) {
  ...
  while (stack.length) {
    ...
    if (isWin(state) || tryGreedyFinishIfReady(state)) {
      ...
      return {
        solvable: true,
        difficulty,
        stats: { nodes, depth: bestDepth, timeMs, cutoffReason: null },
      }
    }
  ...
  return {
    solvable: false,
    difficulty: undefined,
    stats: { nodes, depth: bestDepth, timeMs, cutoffReason },
  }
}
