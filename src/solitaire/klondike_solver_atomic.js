...
const ZOBRIST = (() => {
  ...
  const totalCards = 52
  return {
    foundation: Array.from({ length: suits }, () =>
      Array.from({ length: ranksPlusEmpty }, () => rand64())
    ),
    facedown: Array.from({ length: columns }, () =>
      Array.from({ length: faceDownMax + 1 }, () => rand64())
    ),
    columnCard: Array.from({ length: columns }, () =>
      Array.from({ length: totalCards }, () => rand64())
    ),
...
function cardIdOf(card) {
  return CARD_IDS.get(card.suit + ':' + card.rank)
}
...
function zobristKey(state) {
  ...
  for (let i = 0; i < state.stock.length; i += 1)
    key ^= ZOBRIST.stockCard[cardIdOf(state.stock[i])]
  return key.toString()
}
...
function setupStateFromDeckOrder(deckOrder) {
  if (!Array.isArray(deckOrder) || deckOrder.length !== 52)
    throw new Error('deckOrder must be an array of 52 integers')
  const baseDeck = createDeck()
  const deck = deckOrder.map((i) => baseDeck[i])
  const { tableau, stock } = dealTableauFromDeck(deck)
  const foundations = {}
  for (const s of SUITS) foundations[s] = []
  return { tableau, stock, foundations }
}

function colorOf(suit) {
  return RED.has(suit) ? 'red' : 'black'
}
...
function canDropOnTableau(targetColumn, movingStack) {
  ...
  return (
    top.rank === movingStack[0].rank + 1 &&
    colorOf(top.suit) !== colorOf(movingStack[0].suit)
  )
}
...
function isSafeToFoundation(card, foundations) {
  ...
  if (isRed) {
    const b1 = foundations['clubs'].length
      ? foundations['clubs'][foundations['clubs'].length - 1].rank
      : 0
    const b2 = foundations['spades'].length
      ? foundations['spades'][foundations['spades'].length - 1].rank
      : 0
    return b1 >= required && b2 >= required
  }
  const r1 = foundations['hearts'].length
    ? foundations['hearts'][foundations['hearts'].length - 1].rank
    : 0
  const r2 = foundations['diamonds'].length
    ? foundations['diamonds'][foundations['diamonds'].length - 1].rank
    : 0
...
function normalizeSafeToFoundationNoFlip(state, allowTableauFlip = true) {
  ...
  while (changed) {
    ...
    for (const s of SUITS) {
      const need = state.foundations[s].length
        ? state.foundations[s][state.foundations[s].length - 1].rank + 1
        : ACE
    ...
    for (let ci = 0; ci < state.tableau.length; ci += 1) {
      ...
      for (const s of SUITS) {
        if (
          canDropOnFoundation(top, state.foundations[s], s) &&
          isSafeToFoundation(top, state.foundations)
        ) {
          col.pop()
...
function listMoves(state) {
  ...
  for (let from = 0; from < state.tableau.length; from += 1) {
    ...
    for (let i = 0; i < col.length; i += 1) {
      ...
      for (let k = 0; k < stack.length - 1; k += 1) {
        const a = stack[k],
          b = stack[k + 1]
        if (!(a.rank === b.rank + 1 && colorOf(a.suit) !== colorOf(b.suit))) {
          ok = false
          break
        }
      }
      ...
      for (let to = 0; to < state.tableau.length; to += 1) {
        if (to === from) continue
        if (canDropOnTableau(state.tableau[to], stack))
          moves.push({ kind: 't2t', from, index: i, to })
      }
      break
    }
  }
  ...
  for (const s of SUITS) {
    ...
    if (idx >= 0) {
      const card = state.stock[idx]
      if (isSafeToFoundation(card, state.foundations))
        moves.push({ kind: 'stock2f', stockIndex: idx, suit: s })
    }
  }
  // stock -> tableau
  for (let si = 0; si < state.stock.length; si += 1) {
    ...
    for (let to = 0; to < state.tableau.length; to += 1)
      if (canDropOnTableau(state.tableau[to], [card]))
        moves.push({ kind: 'stock2t', stockIndex: si, to })
  }
...
function cloneState(state) {
  return {
    ...
    foundations: (() => {
      const f = {}
      for (const s of SUITS)
        f[s] = state.foundations[s].map((c) => ({ suit: c.suit, rank: c.rank }))
      return f
    })(),
  }
}
...
function applyMoveClone(state, move) {
  ...
  if (move.kind === 't2f') {
    ...
    s.foundations[move.suit].push({
      suit: state.tableau[move.ci][state.tableau[move.ci].length - 1].suit,
      rank: state.tableau[move.ci][state.tableau[move.ci].length - 1].rank,
    })
  ...
  if (move.kind === 't2t') {
    ...
    if (newTopIndex >= 0 && !s.tableau[move.from][newTopIndex].faceUp) {
      s.tableau[move.from][newTopIndex].faceUp = true
      flipped = true
    }
    return { state: s, flipped }
  }
...
function isAutoCompleteReady(state) {
  return state.tableau.every((col) => col.every((c) => c.faceUp))
}
function isWin(state) {
  let total = 0
  for (const s of SUITS) total += state.foundations[s].length
  return total === 52
}
...
function tryGreedyFinishIfReady(state) {
  ...
  while (changed) {
    ...
    for (let ci = 0; ci < greedy.tableau.length; ci += 1) {
      ...
      const top = col[col.length - 1]
      for (const s of SUITS) {
        if (canDropOnFoundation(top, greedy.foundations[s], s)) {
          col.pop()
          greedy.foundations[s].push({ suit: top.suit, rank: top.rank })
          changed = true
          break
        }
      }
    ...
    for (const s of SUITS) {
      const need = greedy.foundations[s].length
        ? greedy.foundations[s][greedy.foundations[s].length - 1].rank + 1
        : ACE
      const idx = greedy.stock.findIndex((c) => c.suit === s && c.rank === need)
      if (idx >= 0) {
        const card = greedy.stock.splice(idx, 1)[0]
        greedy.foundations[s].push({ suit: card.suit, rank: card.rank })
        changed = true
      }
    }
  }
...
function findNextFlipCandidates(rootState, budget) {
  ...
  function runBfsWithFilter(moveFilter, capOverride, relevantOverride) {
    ...
    function countPathStatsForFilter(path) {
      let stockUses = 0,
        f2tUses = 0
      for (const m of path) {
    ...
    while (q2.length) {
      ...
      for (const mv of allMoves) {
        ...
        if (avoidEmptyUnlessKing && mv.kind === 't2t' && mv.index === 0) {
          ...
          const kingToEmpty =
            moving.length > 0 && moving[0].rank === KING && toCol.length === 0
          if (!kingToEmpty) {
            deferred.push(mv)
            continue
          }
      ...
      for (const mv of movesOrdered) {
        ...
        if (flipped || normFlipped) {
          ...
          const { stockUses, f2tUses } = countPathStatsForFilter(fullPath)
          ...
          if (
            !prev ||
            stockUses < prev.stockUses ||
            (stockUses === prev.stockUses &&
              (f2tUses < prev.f2tUses ||
                (f2tUses === prev.f2tUses && cand.steps < prev.steps)))
          ) {
        ...
        if (!seen2.has(key)) {
          seen2.add(key)
          q2.push({ state: nxt, steps: cur.steps + 1, path: cur.path.concat([mv]) })
        }
      }
    }
  ...
  if (relevantFirst) {
    const capA =
      relevanceStepsCap !== undefined
        ? relevanceStepsCap
        : maxApproachSteps !== undefined
          ? Math.min(maxApproachSteps, 8)
          : 8
  ...
  if (trivialFirst) {
    ...
    const trivial2 = runBfsWithFilter(
      (mv) =>
        mv.kind === 't2f' ||
        mv.kind === 'stock2f' ||
        mv.kind === 'stock2t' ||
  ...
  while (q.length) {
    ...
    for (const mv of movesOrdered) {
      ...
      if (!seen.has(key)) {
        seen.add(key)
        q.push({ state: nxt, steps: cur.steps + 1, path: cur.path.concat([mv]) })
      }
    }
  }
...
function rankCandidates(rootState, candidates) {
  ...
  const getScore = (c) => {
    ...
    const s = neededScoreFromNeeds(needs, c.state)
    cache.set(c, s)
    return s
  }
...
function rankCandidatesBlended(rootState, candidates) {
  ...
  return candidates.slice().sort((a, b) => {
    ...
    if (a.steps !== b.steps) return a.steps - b.steps
    const sa = a.stockUses ?? 0,
      sb = b.stockUses ?? 0
    if (sa !== sb) return sa - sb
    const fa = a.f2tUses ?? 0,
      fb = b.f2tUses ?? 0
...
function totalFoundationCount(state) {
  let t = 0
  for (const s of SUITS) t += state.foundations[s].length
  return t
}
function emptyColumns(state) {
  let t = 0
  for (const col of state.tableau) if (col.length === 0) t += 1
  return t
}
...
function neededScoreFromNeeds(needs, candState) {
  ...
  for (const need of needs) {
    ...
    for (const t of tops) {
      ...
      if (t.rank === need.rank && tColor === need.color) {
        satisfied = true
        break
      }
    }
    ...
    if (lastDown >= 0 && lastDown + 1 < col.length) {
      ...
      if (base.rank === KING) {
        ...
        frontiers.push({
          col: c,
          baseIdx,
          need: { type: 'rank', rank: base.rank + 1, color: needColor },
        })
      }
    }
  }
...
function solveKlondikeAtomic(deckOrder, options = {}) {
  const {
    maxNodes = 2000000,
    maxTimeMs = 5000,
    maxLocalNodes = 200000,
    maxApproachSteps = 20,
    maxApproachStepsHigh = 80,
    relaxAtDepth = 17,
    approachStepsIncrement = 3,
    avoidEmptyUnlessKing = true,
    enableBackjump = true,
    rankingStrategy = 'blended',
    useNeededRanks = false,
    useCoverCanonical = false,
    frontierOnlyMoves = false,
    frontierStageFirst = false,
    relevanceStepsCap,
    maxAtomicCache = 200000,
  } = options
  ...
  function pushAtomicFrame() {
    ...
    if (!entry) {
      const budget = {
        maxLocalNodes,
        deadline,
        maxApproachSteps: localCap,
        avoidEmptyUnlessKing,
        relevantOnly: !!frontierOnlyMoves,
        relevantFirst: !!frontierStageFirst,
        relevanceStepsCap,
      }
      ...
      else if (rankingStrategy === 'mostCovered')
        sorted = rankCandidatesByMostCovered(state, candidates)
    ...
    const allTried =
      entry.candidates.length === 0 || entry.tried.size >= entry.candidates.length
    if (allTried) {
      atomicDead += 1
      return { solved: false, candidates: [], key, snapshot }
    }
  ...
  if (tryGreedyFinishIfReady(state)) {
    return {
      solvable: true,
      difficulty: classifyDifficultyByNodes(0),
      stats: { nodes: 0, depth: 0, timeMs: 0, cutoffReason: null },
    }
  }
  ...
  if (init.solved)
    return {
      solvable: true,
      difficulty: classifyDifficultyByNodes(nodes),
      stats: {
        nodes,
        depth: flipsDepth,
        timeMs: Math.max(0, Date.now() - (deadline - maxTimeMs)),
        cutoffReason: null,
      },
    }
  frames.push({
    ...
    snapshot: init.snapshot || cloneState(state),
    changedCols: new Set(),
  })
  ...
  while (frames.length) {
    ...
    while (top.idx < top.candidates.length) {
      const trialCacheEntry = top.key ? atomicCache.get(top.key) : undefined
      if (!trialCacheEntry || !trialCacheEntry.tried.has(top.idx)) {
        break
      }
      top.idx += 1
    }
    ...
    if (top.idx >= top.candidates.length) {
      ...
      if (!enableBackjump) {
        ...
        if (frames.length) {
          ...
          state.tableau = snap.tableau.map((col) =>
            col.map((c) => ({ suit: c.suit, rank: c.rank, faceUp: c.faceUp }))
          )
          ...
          state.foundations = (() => {
            const f = {}
            for (const s of SUITS)
              f[s] = snap.foundations[s].map((c) => ({ suit: c.suit, rank: c.rank }))
            return f
          })()
        }
        continue
      ...
      while (jumpTo >= 0) {
        ...
        if (anc.changedCols) {
          for (const c of currentBlocked) {
            if (anc.changedCols.has(c)) {
              intersects = true
              break
            }
          }
      ...
      while (frames.length - 1 > targetIdx) {
        frames.pop()
        flipsDepth = Math.max(0, flipsDepth - 1)
      }
      if (frames.length) {
    ...
    state.tableau = base.tableau.map((col) =>
      col.map((c) => ({ suit: c.suit, rank: c.rank, faceUp: c.faceUp }))
    )
    ...
    state.foundations = (() => {
      const f = {}
      for (const s of SUITS)
        f[s] = base.foundations[s].map((c) => ({ suit: c.suit, rank: c.rank }))
      return f
    })()
    for (const mv of cand.path) {
      const { state: s2 } = applyMoveClone(state, mv)
      state.tableau = s2.tableau
      state.stock = s2.stock
      state.foundations = s2.foundations
      nodes += 1
    }
    flipsDepth += 1
    ...
    frames.push({
      ...
      snapshot: next.snapshot || cloneState(state),
      changedCols: changedColumnsFromPath(cand.path),
    })
  }
...
function getAtomicFrameFromState(state, options = {}) {
  ...
  const budget = {
    ...
    maxApproachStepsHigh,
    relaxAtDepth,
    relevantOnly: !!options.frontierOnlyMoves,
    relevantFirst: !!options.frontierStageFirst,
    relevanceStepsCap: options.relevanceStepsCap,
  }
  ...
  else if (strategy === 'mostCovered')
    ranked = rankCandidatesByMostCovered(snapshot, candidates)
