...
const {
  solveKlondike,
  setupStateFromDeckOrder,
} = require('../src/solitaire/klondike_solver')
...
function tableauKeyFromConfig(tableau) {
  const suitCode = (s) =>
    s === 'clubs' ? 'c' : s === 'diamonds' ? 'd' : s === 'hearts' ? 'h' : 's'
...
function solveWithBudgets(deckOrder) {
  ...
  return {
    ...
    preferred:
      atomic && atomic.solvable ? 'atomic' : dfs && dfs.solvable ? 'dfs' : 'none',
  }
}
...
;(async () => {
  ...
  for (
    let i = 0;
    i < shuffles.length && found < targetSolved && evaluated < EVAL_CAP;
    i += 1
  ) {
    ...
    console.log(
      `[harvest] #${i + 1} ${atomicLine} | ${dfsLine} | appended=${appended ? 'yes' : 'no'} | solved=${found}/${targetSolved}`
    )
  }
  ...
  while (found < targetSolved && evaluated < EVAL_CAP) {
    ...
    console.log(
      `[harvest] RNG ${atomicLine} | ${dfsLine} | appended=${appended ? 'yes' : 'no'} | solved=${found}/${targetSolved}`
    )
  }

  console.log(
    `[harvest] Completed. Evaluated=${evaluated}, Added=${found}, Target=${targetSolved}`
  )
  if (!isTrial && found < targetSolved) {
    console.log(
      '[harvest] Target not reached; consider re-running to continue appending more entries.'
    )
  }
})().catch((err) => {
