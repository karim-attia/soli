...
function writeComparisonMd(filePath, decks, resA, resB, labelA, labelB) {
  ...
  for (let i = 0; i < decks.length; i += 1) {
    const a = resA[i],
      b = resB[i]
    const same =
      a.solvable === b.solvable &&
      (a.stats.cutoffReason || '-') === (b.stats.cutoffReason || '-')
    lines.push(
      `| ${i + 1} | ${a.solvable ? '✅' : '❌'} | ${a.stats.cutoffReason || '-'} | ${a.stats.nodes} | ${b.solvable ? '✅' : '❌'} | ${b.stats.cutoffReason || '-'} | ${b.stats.nodes} | ${same ? '✅' : '❌'} |`
    )
  }
...
function main() {
  ...
  const avoidEmptyUnlessKing =
    args.avoidEmptyUnlessKing !== undefined ? !!args.avoidEmptyUnlessKing : undefined
  const enableBackjump =
    args.enableBackjump !== undefined ? !!args.enableBackjump : undefined
  const maxApproachSteps =
    args.maxApproachSteps !== undefined ? Number(args.maxApproachSteps) : undefined
  const maxApproachStepsHigh =
    args.maxApproachStepsHigh !== undefined
      ? Number(args.maxApproachStepsHigh)
      : undefined
  ...
  const useNeededRanks =
    args.useNeededRanks !== undefined ? !!args.useNeededRanks : undefined
  const useCoverCanonical =
    args.useCoverCanonical !== undefined ? !!args.useCoverCanonical : undefined
  const frontierOnlyMoves =
    args.frontierOnlyMoves !== undefined ? !!args.frontierOnlyMoves : undefined
  const frontierStageFirst =
    args.frontierStageFirst !== undefined ? !!args.frontierStageFirst : undefined
  const relevanceStepsCap =
    args.relevanceStepsCap !== undefined ? Number(args.relevanceStepsCap) : undefined
  ...
  const resolved = {
    ...
    avoidEmptyUnlessKing:
      avoidEmptyUnlessKing === undefined ? '(default)' : avoidEmptyUnlessKing,
    ...
    maxApproachStepsHigh:
      maxApproachStepsHigh === undefined ? '(default)' : maxApproachStepsHigh,
    ...
    useCoverCanonical:
      useCoverCanonical === undefined ? '(default: false)' : useCoverCanonical,
    frontierOnlyMoves:
      frontierOnlyMoves === undefined ? '(default: false)' : frontierOnlyMoves,
    frontierStageFirst:
      frontierStageFirst === undefined ? '(default: false)' : frontierStageFirst,
    relevanceStepsCap:
      relevanceStepsCap === undefined ? '(default: 8)' : relevanceStepsCap,
  }
  /* eslint-disable no-console */
  ...
  if (args.compareAtomicVariants) {
    ...
    const atomicLeast = runForDecks(decks, {
      ...baseOpts,
      strategy: 'atomic',
      rankingStrategy: 'leastSteps',
    })
    const atomicMost = runForDecks(decks, {
      ...baseOpts,
      strategy: 'atomic',
      rankingStrategy: 'mostCovered',
    })
    writeComparisonMdAppend(
      saveMd,
      decks,
      atomicLeast,
      atomicMost,
  ...
  for (let t = 0; t < trials; t += 1) {
    ...
    const res = solveKlondike(deckOrder, {
      maxNodes,
      maxTimeMs,
      strategy,
      avoidEmptyUnlessKing,
    ...
    const tried =
      res.stats.atomicTried !== undefined ? ` atomicTried=${res.stats.atomicTried}` : ''
    const dead =
      res.stats.atomicDead !== undefined ? ` atomicDead=${res.stats.atomicDead}` : ''
    const mark = res.solvable ? '✅' : '❌'
    console.log(
      `${mark} Trial ${t + 1}: nodes=${res.stats.nodes} depth=${res.stats.depth} time=${res.stats.timeMs}ms cutoff=${res.stats.cutoffReason || '-'}${tried}${dead}`
    )
    results.push(res)
  }
