...
| 2025-12-14 20:27:47 | significant_update | Review | InProgress | User request: hide draw/empty outlines (stock + tableau empty slots) once the game is won (celebration). | ai_agent |
| 2025-12-14 20:28:17 | submit_for_review | InProgress | Review | Hide stock/waste empty outlines and tableau empty-slot outlines when `hasWon` is true. | ai_agent |
...
- Hide draw/empty outlines (stock + tableau empty slots) when the game is won so celebration visuals are clean.
