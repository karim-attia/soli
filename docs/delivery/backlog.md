...
| 5 | Auto-complete fan | As a player I want the auto-to-foundation flow to be fast and stable so that finishing a game feels smooth. | InReview | 1. Auto-complete animates cards using the same paths as manual moves.<br>2. When auto-moving through the stock, eligible tableau moves backfill automatically to accelerate completion.<br>3. Cards already on foundations no longer wiggle when tapped during auto-complete.<br>[View Details](./5/prd.md) |
