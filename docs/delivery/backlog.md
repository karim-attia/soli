...
| 17 | Returning player | As a player I want miscellaneous polish fixes bundled together so that the game stays stable and responsive. | InProgress | 1. Waste fan draw animation no longer overshoots and eases smoothly.<br>2. App launch avoids layout shifts before the first frame becomes interactive.<br>3. Undo button remains hidden until the first move occurs.<br>4. Session resumes ongoing games without replaying animations while completed games start fresh.<br>5. Toast notifications are disabled until redesigned.<br>6. Undo scrubbing restores previous states without increasing move counts and allows unlimited undos.<br>[View Details](./17/prd.md) |
...
| 20 | Time traveler | As a player I want to scrub through game history by long-pressing undo so that I can rewind or fast-forward rapidly. | InProgress | 1. Long-pressing the undo control dims the button and reveals a full-width Tamagui slider overlay dedicated to timeline scrubbing.<br>2. The slider’s leftmost position restores the initial deal, the rightmost position restores the most recent confirmed action, and intermediate positions replay the corresponding board state while dragging.<br>3. Releasing the slider commits the highlighted state, restores the standard controls, and allows forward scrubbing as long as no new actions were taken since entering the slider.<br>[View Details](./20/prd.md) |
...
| 25 | Visual QA lead | As a player I want cards to retain the calibrated phone proportions so that ranks and suits look identical across devices. | InProgress | 1. Developer logging captures a single snapshot of face-up card metrics plus content style offsets for reference.<br>2. Running `yarn demo:auto-solve` on the connected phone prints the measured width, height, and radius for the reference card used to derive ratios.<br>3. `CardVisual` computes corner labels, suit placement, and font sizes from reference ratios so visuals stay proportional for any `CardMetrics`.<br>4. An end-to-end CoS test reruns the demo auto-solve flow to confirm the on-device proportions remain matched.<br>[View Details](./25/prd.md) |
| 26 | Layering QA lead | As a player I want card flight animations to always appear above other UI so that moves stay readable. | InProgress | 1. `docs/delivery/26/animation-layering-research.md` captures a sourced analysis of React Native/Tamagui layering, z-index behaviour across component trees, and portal strategies.<br>2. Card flights for stock→waste, waste→tableau, tableau→tableau, tableau→foundation, foundation→tableau, and stock→foundation all render on top of overlapping UI in-app builds.<br>3. Automation strategy for verifying every flight combination is documented with implementation hooks and is referenced from `docs/delivery/26/tasks.md`.<br>4. PBI 26 documentation (backlog row, PRD, tasks, and task files) remains synchronized per policy.<br>[View Details](./26/prd.md) |
| 27 | Animation enthusiast | As a player I want to control the display refresh rate so that card animations are consistently smooth regardless of system defaults. | InProgress | 1. Settings includes a "Display refresh rate" control in the Advanced section with High/Auto options.<br>2. Selecting "High" (default) requests max available refresh rate from Android; "Auto" defers to system defaults.<br>3. The setting persists across app restarts and applies on startup.<br>4. The feature only appears on Android devices with >60Hz displays and gracefully no-ops on iOS/web.<br>[View Details](./27/prd.md) |
| 28 | Speedrun player | As a player I want auto-up (auto-complete) to finish much faster and stop counting time once auto-up begins so that end-of-game cleanup feels instant and “time” reflects manual play only. | InProgress | 1. Auto-up completes significantly faster than before.<br>2. The timer stops immediately when auto-up begins and does not restart during auto-up.<br>3. Auto-up draw flights originate from the correct stock position (no off-screen origins).<br>4. Documentation and tasks stay synchronized per policy.<br>[View Details](./28/prd.md) |
| 29 | Mobile solitaire player | As a player I want tableau taps to be more forgiving so that I can move the intended card even when tableau stacks are tightly spaced. | InProgress | 1. When tapping a face-up card in a tableau column, if that card has no valid auto-move, the app tries the adjacent card in the same column (above first, then below) and executes its auto-move if available.<br>2. This fallback only applies to tableau columns (not stock/waste/foundations).<br>3. The fallback never attempts to auto-move face-down cards.<br>4. Documentation and tasks stay synchronized per policy.<br>[View Details](./29/prd.md) |
| 30 | Build engineer | As a developer I want the project upgraded to Expo SDK 55 (including React/React Native alignment) so that the app stays on supported tooling and build diagnostics remain healthy. | InReview | 1. Core SDK dependencies align with Expo SDK 55 expectations and `npx expo install --check` reports up to date.<br>2. React and React Native are upgraded to SDK 55-compatible versions and TypeScript/test checks pass.<br>3. Web export succeeds under SDK 55 without runtime build errors.<br>4. Migration documentation includes sourced package guides and a step-by-step findings log.<br>[View Details](./30/prd.md) |
...
| 20251107-100500 | 20 | create_pbi | Created PBI 20 for undo timeline scrubbing via long press. | ai_agent |
| 20251107-100600 | 20 | propose_for_backlog | User approved undo timeline scrubber scope for prioritisation. | user |
| 20251107-100930 | 20 | start_implementation | Began implementation focusing on timeline scrubbing UI (Task 20-1). | ai_agent |
| 2025-11-12 09:45:00 | 21 | create_pbi | Logged solvable shuffle fairness improvements to prevent repeat deals after skips. | ai_agent |
| 2025-11-12 09:46:00 | 21 | propose_for_backlog | User approved solvable shuffle fairness scope. | user |
| 2025-11-12 09:47:00 | 21 | start_implementation | Began implementing solvable shuffle fairness improvements (Task 21-1). | ai_agent |
| 20251108-093000 | 17 | start_implementation | Retitled PBI 17 for miscellaneous polish coverage and kicked off Task 17-9 for undo move handling. | ai_agent |
| 20251112-103000 | 22 | create_pbi | Created PBI 22 for stack rendering refactor and waste animation polish. | ai_agent |
| 20251112-103100 | 22 | propose_for_backlog | User approved PBI 22 scope via refactor request. | user |
| 20251112-103200 | 22 | start_implementation | Began implementation on PBI 22 tasks 22-1 through 22-3. | ai_agent |
| 20251112-110900 | 22 | submit_for_review | Submitted per-card stock rendering and animation tweaks for review. | ai_agent |
| 20251112-111000 | 22 | approve | User approved PBI 22 implementation and verification results. | user |
...
| 2025-11-14 11:05:00 | 25 | create_pbi | Logged card visual proportionality calibration request. | ai_agent |
| 2025-11-14 11:06:00 | 25 | propose_for_backlog | User approved proportional card visual scope. | user |
| 2025-11-14 11:07:00 | 25 | start_implementation | Began implementation under tasks 25-1 and 25-2. | ai_agent |
| 20251123-090000 | 26 | create_pbi | Logged PBI 26 to resolve card flight layering and z-index issues. | ai_agent |
| 20251123-090100 | 26 | propose_for_backlog | User prioritized PBI 26 to investigate and fix animation layering. | user |
| 20251123-090200 | 26 | start_implementation | Began research and implementation tasks for PBI 26. | ai_agent |
| 20251205-100000 | 27 | create_pbi | Created PBI 27 for Android high refresh rate control. | ai_agent |
| 20251205-100100 | 27 | propose_for_backlog | User approved refresh rate control scope. | user |
| 20251205-100200 | 27 | start_implementation | Began implementation of refresh rate native module and settings. | ai_agent |
| 20251214-124000 | 28 | create_pbi | Created PBI 28 to speed up auto-up and stop timer when automation starts. | ai_agent |
| 20251214-124100 | 28 | propose_for_backlog | User approved scope for much faster auto-up and timer-stop-on-auto-up-start. | user |
| 20251214-124200 | 28 | start_implementation | Began implementing faster auto-up cadence and timer behavior. | ai_agent |
| 20251214-211000 | 29 | create_pbi | Created PBI 29 to make tableau taps more forgiving by retrying adjacent cards on invalid auto-move taps. | ai_agent |
| 20251214-211100 | 29 | propose_for_backlog | User requested adjacent-card fallback behaviour for tableau tap-to-auto-move. | user |
| 20251214-211200 | 29 | start_implementation | Began implementing tableau tap fallback under PBI 29 tasks. | ai_agent |
| 2026-03-02 00:05:00 | 30 | create_pbi | Created PBI 30 for Expo SDK 55 + React/React Native alignment and validation. | ai_agent |
| 2026-03-02 00:06:00 | 30 | propose_for_backlog | User requested web research and full implementation of the Expo 55 upgrade path. | user |
| 2026-03-02 00:07:00 | 30 | start_implementation | Began dependency audit, official-doc research, and migration execution for SDK 55. | ai_agent |
| 2026-03-02 00:58:00 | 30 | submit_for_review | SDK 55 dependency alignment, compatibility fix, and validation completed; awaiting review. | ai_agent |
| 2026-03-02 21:29:00 | 30 | significant_update | Reopened validation after reported Java/Gradle and Android device runtime issues during post-upgrade testing. | user |
| 2026-03-02 22:30:00 | 30 | submit_for_review | Navigator registration issue resolved, dependency alignment revalidated, and on-device gameplay smoke passed using agent-device. | ai_agent |
