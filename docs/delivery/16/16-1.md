...
| 2025-11-04 09:35:00 | Status Change | InProgress | Review | Developer mode toggle, gating, and demo game ready for review. | ai_agent |
...
- Run `npx expo run:android --variant release` with a device attached, then `npx uri-scheme open "soli://demo-game?auto=1" --android` to confirm the release build loads the demo and auto-reveals the two hidden cards.
...
- **Install + launch + verify**:
  ...
  npx expo run:android --variant release \
    && adb logcat -c \
    && npx uri-scheme open "soli:///?demo=1&auto=1&solve=1" --android \
    && adb logcat -d | grep "Foundations complete"
  ```
  - The `solve=1` flag auto-plays the demo through to victory so the new log entry appears.
  - Omit `solve=1` (or set `solve=0`) if you prefer to finish the demo manually.
