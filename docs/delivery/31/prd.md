...
- Analyze the established Oxc/tsgo project configuration and adapt it to current TypeScript 7 and Expo SDK 56 guidance.
- Use one Expo-based `tsconfig.json` for the native TypeScript checker rather than maintaining parallel `tsc` and `tsgo` configurations.
- Add Oxc and tsgo dev dependencies plus scripts for lint/fix/format/check/typecheck.
...
1. `package.json` includes scripts for `format`, `format:check`, `lint`, `lint:fix`, `lint:strict`, and a direct `tsgo`-backed `typecheck`.
2. `.oxlintrc.json`, `.oxfmtrc.json`, and the unified Expo-based `tsconfig.json` are aligned to project needs and TypeScript 7.
...
- `oxlint`
- `oxfmt`
- Expo SDK 56 TypeScript base configuration
