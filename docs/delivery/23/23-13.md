...
| 2025-11-12 20:14:00 | Status Change | Review | InProgress | Reopened to explore latest UI vs state decoupling questions. | ai_agent |
| 2025-11-12 20:26:00 | Status Change | InProgress | Review | Added follow-up analysis covering selection helpers and decoupling plan. | ai_agent |
...
- **Selection resolver context** *(new)*: Documenting why `collectSelectionCardIds`/`resolveSelectionCardIds` remain—namely to let the flight controller await layout snapshots for auto-move/auto-complete flows—even after manual selection removal clarifies their current purpose.
- **Additional extraction candidates** *(new)*: Beyond the auto-queue hook, relocating history recording, statistics view-model, and undo orchestration into feature hooks/components further shrinks the screen. These moves also make it easier to encapsulate the UI/game-state split described above.

## Outcome
- Documented concrete extraction opportunities and an outline for fully decoupling UI from state orchestration.
- Expanded the summary to answer the latest questions about selection utilities and next extraction steps.
