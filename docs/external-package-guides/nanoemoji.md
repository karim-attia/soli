...
- [Nanoemoji releases](https://github.com/googlefonts/nanoemoji/releases)
- [Nanoemoji PyPI](https://pypi.org/project/nanoemoji/)
- [Noto Emoji README](https://github.com/googlefonts/noto-emoji)
- [Noto Emoji rebuild script](https://github.com/googlefonts/noto-emoji/blob/main/full_rebuild.sh)
- [Google Fonts color font guide](https://googlefonts.github.io/gf-guide/color.html)
- [Noto Color Emoji on Google Fonts](https://fonts.google.com/noto/specimen/Noto%2BColor%2BEmoji)
...
- Status: still useful for the historical color-font experiment; current shipping
  card text remains font-only and should not resurrect this path without a new
  visual validation plan.
- PyPI/GitHub show `nanoemoji==0.15.9` as current. The old install command remains
  valid, but pin the version in a temporary virtualenv if reproducing the March
  2026 outputs.
- `maximum_color` remains the relevant helper for adding SVG to COLR or COLR to
  SVG. Release notes since v0.15.1 also mention `--colr_version` for generating
  COLRv0 from an OT-SVG color font; use that only if a future platform test shows
  COLRv0 is needed.
- Upstream naming is easy to misread: the Noto Emoji README still describes
  `NotoColorEmoji.ttf` as CBDT/CBLC, while Google Fonts and the Noto build script
  also expose COLRv1/SVG-era outputs such as `Noto-COLRv1.ttf`. Always inspect the
  actual font tables from the device or downloaded asset before deciding which
  color-font path applies.
