...
- [x] C12 (2026-07-08). Skill pruned to best-practice form + deeplink self-heal. Skill rewritten per Karim's principle (see learnings): dated changelog markers removed, initial-URL-replay historical-workaround paragraph deleted (fix ships in every build), force-stop/#retry-N caveats collapsed into the raw-one-liners note (the wrapper handles both), section 9 now encodes replace-don't-append, `docs/product/agent-testing-skill` added to further reading. Size: 181 → 146 lines / 18.3 → 14.6 KB (~20%; short of the 25–35% aim because the mandated keep-list — a11y matrix, guardrail ladder, contested-sim recipe, catalog + shortcut tables, troubleshooting — dominates the file; everything cut was changelog framing, solved-state workarounds, or duplication). `deeplink.js`: android target with no device now runs `scripts/android-ready.sh` automatically (stdio inherited, one serial-resolution retry) instead of erroring with instructions; failure after that points at `--ios`.
- [x] C8. Skill update — DONE. Section 3 catalog gained all five link families with when-to-use notes + a "Celebration testing" subsection; section 4 gained reset=undoHint/reset=game + settings link + no-reset-all rationale; decision-tree fixture list gained nearwin and exact-deal entries. (Celebration note updated same evening for the controller-owned dev-hold preview semantics.)
...
- [x] C11 (2026-07-08, approved by Karim). Phone readiness tooling — `scripts/android-ready.sh` (new): resolves target (`ANDROID_SERIAL` > sole device > bounded ~2 min wireless reconnect via `ADB_WIFI_TARGET`/`ADB_WIFI_IP` + mdns, mirroring run-android-release.sh discovery), wakes (KEYCODE_WAKEUP), unlocks via the gitignored `android-unlock-pattern.sh` when present (graceful note when missing), `--keep-awake` raises screen-off timeout to 600000 ms (same constant as `--auto-solve`) and prints the exact restore command — no daemons, no `svc power stayon`. Ends with a one-line summary (serial · mWakefulness · keyguard state). `deeplink.js` android path now preflights wake+unlock inline (cheap subset, NO reconnect loop — fast path stays fast) and its no-device error points at android-ready.sh / `--ios`. Skill: build-section unlock line reworded (yarn release wakes/unlocks automatically — was misread as a manual pre-step), section 5 + troubleshooting now say "run android-ready.sh, NEVER hand-roll wake/reconnect loops". Verified: `bash -n` clean, real run on the phone (reconnect→wake→unlock→summary), `--ios` deeplink smoke. All files staged per Karim. Section 3 catalog gained all five link families with when-to-use notes + a "Celebration testing" subsection (abort/dialog gotchas: tap = abort → forced Deal Again; dialog auto-opens at 30 s); section 4 gained reset=undoHint/reset=game + settings link + no-reset-all rationale; decision-tree fixture list gained nearwin and exact-deal entries.

- [x] C13 (2026-07-08, approved by Karim). Unify adb discovery: extract the battle-tested discovery/connect/pinning functions from `run-android-release.sh` verbatim into a shared sourced lib (`scripts/lib/adb-discovery.sh`) with a single `discover_and_connect`-style entry; `run-android-release.sh` sources it (identical behavior); `scripts/android-ready.sh` drops its simplified copies (pick_adb, env load, serial listing, mDNS reconnect loop) and uses the full shared flow incl. adb-server ownership repair and tcpip :5555 pinning (pinning short-circuits when already pinned, degrades gracefully — cheap for the driving loop, and port rotation is the root cause of the doze/drop problem android-ready.sh exists for). Rationale (Karim): one implementation = fewer issues + one place to change process in future; earlier "high risk" framing was wrong because functions move verbatim and pinning is a no-op on the happy path. `deeplink.js` unchanged. Verify: bash -n / shellcheck, real `android-ready.sh` run, one end-to-end `yarn release`. DONE 2026-07-08 — `scripts/lib/adb-discovery.sh` (new, 462 lines: 14 verbatim functions + parameterized `load_env_file` + new `discover_and_connect_device` orchestrator encapsulating the discovery→restart-retry→connect→restart-retry→connect sequencing formerly inlined in main()); `run-android-release.sh` 616 → 197 lines (sources lib, main() calls the orchestrator, everything else byte-identical — verified per-function diff vs HEAD); `scripts/android-ready.sh` 159 → 111 lines (local copies deleted, keeps its own multi-device guard + ANDROID_SERIAL short-circuit, now runs ensure_adb_server_owner + full shared discovery + pin_tcpip_port before wake/unlock). Verified: bash -n clean (shellcheck not installed on this machine), real android-ready run (happy path, already-pinned 192.168.1.12:5555, <2 s), end-to-end yarn release green (32 s incremental: discovery picked the pinned serial, build 25 s, install verified versionCode 13, launch OK — log `.test-artifacts/builds/20260708-133938.log`).
...
- C11 (2026-07-08): `scripts/android-ready.sh` (new — phone reconnect/wake/unlock preflight), `scripts/deeplink.js` (android wake+unlock preflight, error points at android-ready.sh), `.agents/skills/soli-testing/SKILL.md` (build unlock line, section 5 + troubleshooting rows)
- C13 (2026-07-08): `scripts/lib/adb-discovery.sh` (new — shared discovery/connect/pinning lib + `discover_and_connect_device` orchestrator), `run-android-release.sh` (sources lib, moved functions deleted, main() uses orchestrator), `scripts/android-ready.sh` (rewritten on the shared lib: full discovery + server-ownership repair + :5555 pinning; bespoke 120s loop, local pick_adb/env-load/collect_candidates deleted; keeps local multi-device guard), `scripts/deeplink.js` (comment only), `.agents/skills/soli-testing/SKILL.md` (two lines: android-ready now same discovery as yarn release + pin)
...
- Celebration trigger semantics inherited from the real flow: dialog timer auto-opens "Start a new game?" 30 s in; tapping outside the badge aborts AND force-opens the Deal Again dialog. Documented in the skill so agents screenshot before tapping. (Superseded 2026-07-07 by the controller-owned dev-hold preview: no dialogs, silent abort.)
- **Documentation principle (2026-07-08, Karim)**: the skill must read as the ultimate LATEST best practice, not a changelog — no dated change markers, no "per Karim" attributions, no workarounds for already-fixed states. Replace outdated skill content in place; history and rationale live HERE in the plan doc. (This entry is the canonical home for the rationales pruned from the skill below.)
- Android-first test order (2026-07-08, Karim: "makes iteration loop much faster"): test on the phone FIRST when available, stop-and-report Android issues before the iOS pass — fixing first and re-testing once beats discovering the same bug twice. Don't flip back to sim-first.
- `yarn deeplink` default target flipped sim-first → android-first (2026-07-07, Karim): the phone-connected zero-flag case is HIS typical manual use; agents on the simulator must pass `--ios` explicitly because the phone is usually connected and the fallback would hit it. Don't flip back.
- C11 root causes (2026-07-08): agents hand-rolled 2-minute adb connect/wake loops in the celebration chat because (a) the skill's "Unlock a locked phone: android-unlock-pattern.sh" line read like a required manual pre-step even though `yarn release` unlocks automatically (build-tools.js `unlockDevice`), and (b) no tool covered the POST-build iterative-driving loop — the unlock script only handles "at lockscreen", not a dropped wireless adb connection after the phone dozes. Fix: reworded skill line + new `android-ready.sh` recovery command; `--keep-awake` mirrors `--auto-solve`'s 600000 ms screen-off timeout but prints the restore command instead of auto-restoring (no daemon to own the restore).
- C13 unification (2026-07-08, Karim): android-ready.sh's C11 discovery was a weaker duplicate of run-android-release.sh's (no server-ownership repair, no alias-serial lsof resolution, no restart-and-retry, no :5555 pinning) — and pinning is exactly what prevents the doze/port-rotation drops android-ready exists for. Decision: ONE implementation in `scripts/lib/adb-discovery.sh`, both entry points source it; the earlier "high risk" framing was wrong because the functions move verbatim and pin_tcpip_port is a no-op when the serial is already `<ip>:5555`. Two deliberate deltas, both commented inline: (a) restart_adb_for_discovery_retry's message generalized from "before release" to "before retrying discovery" (now shared outside release builds — the ONLY non-verbatim change, verified by per-function diff vs HEAD); (b) android-ready keeps a LOCAL multi-device guard because the shared collect_existing_physical_serial deliberately returns the first device on multiples (release flow), while the driving loop must never guess — one device might be Karim's main phone. `load_env_file` is parameterized with the repo-root default derived from the lib's own path.
